...
// // import { Upload, Users, BookOpen, UserPlus, Lock, Smartphone, AlertTriangle, ShieldCheck, X, Save, CheckCircle2, Trash2, Download, BarChart2 } from 'lucide-react';

// // // 🛑 THE SUPERUSER (Change this to your actual email!)
// // const SUPER_ADMIN = "devangsharma0601@gmail.com"; 
...
// //   const [facultyName, setFacultyName] = useState<string | null>(null); // NEW: To hold logged-in faculty name
  
// //   // Tab States
// //   const [adminTab, setAdminTab] = useState<'faculty' | 'students'>('faculty');
...
// //   // Forms
// //   const [newFacName, setNewFacName] = useState(""); // NEW: For the Add Faculty form
...
// //     // UPDATED: Now fetches 'name' as well
...
// //       fetchFacultyReports(facultyData.subject_name);
// //       return setUserRole('faculty');
// //     }

// //     if (email.endsWith("@uecu.ac.in") && email.split('@')[0].match(/([a-zA-Z]{2})(\d{2})\d{2}(\d{2})$/)) {
...
// //   };
// //   const fetchFacultyReports = async (subject: string) => {
// //     const { data } = await supabase.from('attendance_logs').select('*').eq('subject_name', subject).order('date', { ascending: false });
...
// //     const headers = ["Date", "Roll Number", "MAC Address", "Subject", "Status"];
// //     const csvRows = [headers.join(',')];
    
// //     reportData.forEach(row => {
// //       csvRows.push([row.date, row.roll_number, row.mac_address, row.subject_name, row.status].join(','));
// //     });
...
// //     a.href = url;
// //     a.download = `${facultySubject}_Attendance_Report.csv`;
// //     a.click();
// //   };
...
// //     // UPDATED: Inserts the name as well
// //     const { error } = await supabase.from('faculty').insert([
// //       { name: newFacName, email: newFacEmail, subject_name: newFacSubject }
// //     ]);
...
// //     } else {
// //       setMessage(`Successfully added ${newFacName}`);
// //       setNewFacName(""); 
// //       setNewFacEmail(""); 
// //       setNewFacSubject("");
// //       fetchFaculty(); // Refresh the table instantly
// //     }
...
// //     } else {
// //       table === 'faculty' ? fetchFaculty() : fetchStudents();
// //     }
// //   };
...
// //     formData.append('subject', facultySubject || "General"); 
    
// //     try {
// //       const res = await fetch(`http://127.0.0.1:8000/upload/daily`, { method: 'POST', body: formData });
// //       const data = await res.json();
// //       setMessage(data.message + ` (${data.present_count || 0} Present)`);
// //       fetchFacultyReports(facultySubject!); // Refresh Reports
...
// //       const rollNo = prefix.match(/([a-zA-Z]{2})(\d{2})\d{2}(\d{2})$/)?.[3];

// //       const res = await fetch(`http://127.0.0.1:8000/register/student`, {
// //         method: 'POST',
// //         headers: { 'Content-Type': 'application/json' },
// //         body: JSON.stringify({ mac_address: macInput, roll_number: rollNo })
// //       });
// //       const data = await res.json();
...
// //           <button onClick={handleGoogleLogin} disabled={loading} className="w-full py-4 bg-white text-slate-900 rounded-xl font-bold hover:bg-slate-200 transition-all">Sign in with Google</button>
// //         </div>
// //       </div>
// //     );
// //   }
...
// //           <h1 className="text-2xl font-black text-purple-400 flex items-center gap-2"><ShieldCheck /> Admin Center</h1>
// //           <button onClick={() => supabase.auth.signOut()} className="text-xs bg-slate-800 px-4 py-2 rounded-lg hover:text-white transition-all">Logout</button>
// //         </div>

// //         {/* Admin Navbar */}
// //         <div className="max-w-6xl mx-auto flex gap-4 mb-8 border-b border-slate-800 pb-4">
// //           <button onClick={() => setAdminTab('faculty')} className={`px-4 py-2 font-bold rounded-lg transition-all ${adminTab === 'faculty' ? 'bg-purple-600' : 'text-slate-400 hover:bg-slate-800'}`}>Manage Faculty</button>
// //           <button onClick={() => setAdminTab('students')} className={`px-4 py-2 font-bold rounded-lg transition-all ${adminTab === 'students' ? 'bg-purple-600' : 'text-slate-400 hover:bg-slate-800'}`}>Manage Students</button>
// //         </div>

// //         <div className="max-w-6xl mx-auto">
// //           {adminTab === 'faculty' && (
// //             <div className="grid md:grid-cols-3 gap-6">
// //               <div className="bg-slate-900 p-6 rounded-3xl border border-purple-900/50">
// //                 <h2 className="font-bold mb-4 flex items-center gap-2"><UserPlus size={18}/> Add Faculty</h2>
// //                 <form onSubmit={submitNewFaculty} className="space-y-4">
// //                   {/* NEW: Name Input added here */}
// //                   <input type="text" value={newFacName} onChange={e => setNewFacName(e.target.value)} required placeholder="Professor Name" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl outline-none focus:border-purple-500 text-white" />
// //                   <input type="email" value={newFacEmail} onChange={e => setNewFacEmail(e.target.value)} required placeholder="Email" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl outline-none focus:border-purple-500 text-white" />
// //                   <input type="text" value={newFacSubject} onChange={e => setNewFacSubject(e.target.value)} required placeholder="Subject" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl outline-none focus:border-purple-500 text-white" />
// //                   <button type="submit" className="w-full py-3 bg-purple-600 hover:bg-purple-500 rounded-xl font-bold transition-all">Save Faculty</button>
// //                 </form>
// //               </div>
// //               <div className="md:col-span-2 bg-slate-900 p-6 rounded-3xl border border-slate-800">
// //                 <h2 className="font-bold mb-4">Active Faculty</h2>
// //                 <div className="overflow-x-auto">
// //                   <table className="w-full text-left">
// //                     {/* UPDATED: Added Name header */}
// //                     <thead><tr className="text-slate-400 border-b border-slate-800"><th className="p-3">Name</th><th className="p-3">Email</th><th className="p-3">Subject</th><th className="p-3 text-right">Action</th></tr></thead>
// //                     <tbody>
// //                       {facultyList.length === 0 ? <tr><td colSpan={4} className="p-4 text-center text-slate-500">No faculty found.</td></tr> : 
// //                         facultyList.map(f => (
// //                         <tr key={f.email} className="border-b border-slate-800/50 hover:bg-slate-800/20">
// //                           {/* UPDATED: Added Name column */}
// //                           <td className="p-3 font-bold text-white">{f.name}</td>
// //                           <td className="p-3">{f.email}</td>
// //                           <td className="p-3 text-purple-300">{f.subject_name}</td>
// //                           <td className="p-3 text-right"><button onClick={() => deleteRecord('faculty', 'email', f.email)} className="text-red-500 hover:text-red-400 p-2"><Trash2 size={18}/></button></td>
// //                         </tr>
// //                       ))}
// //                     </tbody>
// //                   </table>
...
// //                  Student Directory 
// //                  <span className="text-xs text-slate-400 bg-slate-950 px-3 py-1 rounded-full border border-slate-800">Read/Delete Mode</span>
// //                </h2>
// //                <div className="overflow-x-auto">
// //                  <table className="w-full text-left">
// //                    <thead><tr className="text-slate-400 border-b border-slate-800"><th className="p-3">Roll</th><th className="p-3">MAC Address</th><th className="p-3 text-right">Action</th></tr></thead>
// //                    <tbody>
// //                      {studentsList.length === 0 ? <tr><td colSpan={3} className="p-4 text-center text-slate-500">No students registered yet.</td></tr> :
// //                        studentsList.map(s => (
// //                        <tr key={s.mac_address} className="border-b border-slate-800/50 hover:bg-slate-800/20">
// //                          <td className="p-3 font-bold">{s.roll_number}</td><td className="p-3 font-mono text-blue-300 text-sm">{s.mac_address}</td>
// //                          <td className="p-3 text-right"><button onClick={() => deleteRecord('students', 'mac_address', s.mac_address)} className="text-red-500 hover:text-red-400 p-2"><Trash2 size={18}/></button></td>
// //                        </tr>
// //                      ))}
// //                    </tbody>
// //                  </table>
...
// //         <div className="max-w-6xl mx-auto flex justify-between items-center mb-6">
// //           <div>
// //             {/* UPDATED: Dynamic personalized greeting */}
// //             <h1 className="text-2xl font-black text-blue-400">Welcome, {facultyName || "Professor"}</h1>
// //             <p className="text-sm text-slate-400">Subject: <strong className="text-white">{facultySubject}</strong></p>
// //           </div>
// //           <button onClick={() => supabase.auth.signOut()} className="text-xs bg-slate-800 px-4 py-2 rounded-lg hover:text-white transition-all">Logout</button>
// //         </div>
...
// //         {/* Faculty Navbar */}
// //         <div className="max-w-6xl mx-auto flex gap-4 mb-8 border-b border-slate-800 pb-4">
// //           <button onClick={() => setFacultyTab('scanner')} className={`px-4 py-2 font-bold rounded-lg flex items-center gap-2 transition-all ${facultyTab === 'scanner' ? 'bg-blue-600' : 'text-slate-400 hover:bg-slate-800'}`}><Upload size={18}/> Scanner</button>
// //           <button onClick={() => { setFacultyTab('reports'); fetchFacultyReports(facultySubject!); }} className={`px-4 py-2 font-bold rounded-lg flex items-center gap-2 transition-all ${facultyTab === 'reports' ? 'bg-blue-600' : 'text-slate-400 hover:bg-slate-800'}`}><BarChart2 size={18}/> Reports</button>
// //         </div>

// //         <div className="max-w-4xl mx-auto">
// //           {facultyTab === 'scanner' && (
// //             <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-xl">
// //               <div className="border-2 border-dashed border-slate-700 rounded-2xl p-12 text-center cursor-pointer hover:border-blue-500/50 bg-slate-950/50 transition-colors">
// //                 <input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} className="hidden" id="csv" />
// //                 <label htmlFor="csv" className="cursor-pointer block text-slate-300 font-medium">{file ? file.name : `Upload ESP8266 CSV for ${facultySubject}`}</label>
// //               </div>
// //               <button onClick={handleFacultyUpload} disabled={loading} className="mt-8 w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold transition-all shadow-lg">Process Attendance</button>
// //               {message && <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl text-blue-300 flex items-center gap-2"><CheckCircle2 size={16}/> {message}</div>}
// //             </div>
// //           )}
...
// //                 <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800">
// //                   <p className="text-slate-400 text-sm">Total Scanned Records</p>
// //                   <h3 className="text-3xl font-black text-blue-400 mt-1">{reportData.length}</h3>
// //                 </div>
// //                 <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex items-center justify-center">
// //                   <button onClick={downloadCSV} className="w-full py-3 bg-green-600/20 text-green-400 border border-green-600/50 hover:bg-green-600 hover:text-white rounded-xl font-bold flex justify-center items-center gap-2 transition-all">
// //                     <Download size={18}/> Export CSV Report
// //                   </button>
// //                 </div>
// //               </div>
// //               <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 max-h-[500px] overflow-y-auto">
// //                 <table className="w-full text-left">
// //                   <thead><tr className="text-slate-400 border-b border-slate-800"><th className="p-3">Date</th><th className="p-3">Roll Number</th><th className="p-3">MAC Address</th></tr></thead>
// //                   <tbody>
// //                     {reportData.length === 0 ? <tr><td colSpan={3} className="p-4 text-center text-slate-500">No attendance data found.</td></tr> :
// //                       reportData.map((r, i) => (
// //                       <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/20 text-sm">
// //                         <td className="p-3">{r.date}</td><td className="p-3 font-bold text-white">{r.roll_number}</td><td className="p-3 font-mono text-slate-500">{r.mac_address}</td>
// //                       </tr>
// //                     ))}
// //                   </tbody>
// //                 </table>
...
// //   if (userRole === 'student') {
// //     const prefix = user.email.split('@')[0];
// //     const match = prefix.match(/([a-zA-Z]{2})(\d{2})\d{2}(\d{2})$/);
// //     const branch = match?.[1].toUpperCase() || "Unknown";
// //     const batch = match?.[2] || "Unknown";
// //     const rollNo = match?.[3] || "Unknown";

// //     return (
// //       <div className="min-h-screen bg-slate-950 text-slate-50 p-6">
// //         <div className="max-w-xl mx-auto flex justify-between items-center mb-6">
// //           <h2 className="text-2xl font-black text-blue-400">Student Portal</h2>
// //           <button onClick={() => supabase.auth.signOut()} className="text-xs bg-slate-800 px-3 py-1 rounded-lg hover:text-white transition-all">Logout</button>
// //         </div>

// //         {/* Student Navbar */}
// //         <div className="max-w-xl mx-auto flex gap-4 mb-8 border-b border-slate-800 pb-4">
// //           <button onClick={() => setStudentTab('profile')} className={`px-4 py-2 font-bold rounded-lg transition-all ${studentTab === 'profile' ? 'bg-blue-600' : 'text-slate-400 hover:bg-slate-800'}`}>My Device</button>
// //           <button onClick={() => setStudentTab('attendance')} className={`px-4 py-2 font-bold rounded-lg transition-all ${studentTab === 'attendance' ? 'bg-blue-600' : 'text-slate-400 hover:bg-slate-800'}`}>My Attendance</button>
// //         </div>
...
// //                 <div className="bg-blue-600/20 p-3 rounded-full text-blue-400"><Smartphone size={24} /></div>
// //                 <div>
// //                   <p className="text-sm text-slate-400">Branch: {branch} | Batch: 20{batch}</p>
// //                   <p className="font-black text-xl">Roll No: {rollNo}</p>
// //                 </div>
// //               </div>
// //               <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-xl">
// //                 <p className="text-slate-400 mb-4 text-sm">Enter your device MAC address (Settings &gt; About Phone &gt; Status).</p>
// //                 <input value={macInput} onChange={(e) => setMacInput(e.target.value.toUpperCase())} placeholder="AA:BB:CC:DD:EE:FF" className="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl mb-4 text-white uppercase font-mono outline-none focus:border-blue-500" />
// //                 <button onClick={handleStudentLink} disabled={loading} className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold transition-all shadow-lg">
// //                   {loading ? "Linking..." : "Save Device Registration"}
// //                 </button>
// //                 {message && <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl text-blue-300 text-sm flex items-center gap-2"><CheckCircle2 size={16}/> {message}</div>}
// //               </div>
// //             </div>
// //           )}
...
// //                <h3 className="text-lg font-bold mb-2">Attendance Syncing</h3>
// //                <p className="text-slate-400 text-sm">Once your device is registered and scanned by faculty, your subject-wise attendance logs will appear here.</p>
// //              </div>
// //           )}
// //         </div>
// //       </div>
...
// //       <div className="bg-slate-900 border border-red-900/50 p-8 rounded-3xl w-full max-w-md shadow-2xl text-center">
// //         <div className="flex justify-center mb-6 text-red-500"><AlertTriangle size={48} /></div>
// //         <h2 className="text-2xl font-black text-white mb-2">Access Denied</h2>
// //         <p className="text-slate-400 mb-8 text-sm">Email not recognized by the system.</p>
// //         <button onClick={() => supabase.auth.signOut()} className="w-full py-4 bg-red-600 hover:bg-red-500 text-white rounded-xl font-bold">Sign Out</button>
// //       </div>
// //     </div>
// //   );
// // }
...
// // const SUPER_ADMIN = "devangsharma0601@gmail.com"; 
// const SUPER_ADMIN = ["harrypotterjee2023@gmail.com", "devangsharma0601@gmail.com"]; // You can add multiple super admins here
...
//     else table === 'faculty' ? fetchFaculty() : fetchStudents();
//   };
...
//       const data = await res.json();
//       setMessage(data.message + ` (${data.present_count || 0} Present)`);
//       fetchFacultyReports(facultySubject!); 
//     } catch { setMessage("Backend Offline"); } 
//     finally { setLoading(false); }
//   };
...
//         {/* RESPONSIVE HEADER: Centers on mobile, splits on desktop */}
//         <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
//           <h1 className="text-2xl font-black text-purple-400 flex items-center gap-2 text-center">
//             <ShieldCheck /> Admin Center
//           </h1>
//           <button onClick={() => supabase.auth.signOut()} className="text-sm bg-slate-800 px-6 py-2 rounded-xl hover:bg-slate-700 transition-all w-full md:w-auto font-medium">
//             Logout
//           </button>
//         </div>

//         {/* RESPONSIVE NAVBAR */}
//         <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-center md:justify-start gap-2 mb-8 border-b border-slate-800 pb-4">
//           <button onClick={() => setAdminTab('faculty')} className={`px-6 py-3 font-bold rounded-xl transition-all w-full md:w-auto ${adminTab === 'faculty' ? 'bg-purple-600' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}>Manage Faculty</button>
//           <button onClick={() => setAdminTab('students')} className={`px-6 py-3 font-bold rounded-xl transition-all w-full md:w-auto ${adminTab === 'students' ? 'bg-purple-600' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}>Manage Students</button>
//         </div>

//         <div className="max-w-6xl mx-auto">
//           {adminTab === 'faculty' && (
//             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
//               <div className="bg-slate-900 p-6 md:p-8 rounded-3xl border border-purple-900/50 h-fit">
//                 <h2 className="font-bold mb-6 flex items-center justify-center md:justify-start gap-2 text-lg"><UserPlus size={20}/> Add Faculty</h2>
//                 <form onSubmit={submitNewFaculty} className="space-y-4">
...
//                   <input type="email" value={newFacEmail} onChange={e => setNewFacEmail(e.target.value)} required placeholder="Email" className="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl outline-none focus:border-purple-500 text-white" />
//                   <input type="text" value={newFacSubject} onChange={e => setNewFacSubject(e.target.value)} required placeholder="Subject" className="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl outline-none focus:border-purple-500 text-white" />
//                   <button type="submit" className="w-full py-4 bg-purple-600 hover:bg-purple-500 rounded-xl font-bold transition-all mt-2">Save Faculty</button>
//                 </form>
//               </div>
//               <div className="lg:col-span-2 bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-800">
//                 <h2 className="font-bold mb-6 text-lg text-center md:text-left">Active Faculty</h2>
//                 <div className="overflow-x-auto w-full rounded-xl">
//                   <table className="w-full text-left min-w-[600px]">
//                     <thead><tr className="text-slate-400 border-b border-slate-800 bg-slate-950"><th className="p-4 rounded-tl-xl">Name</th><th className="p-4">Email</th><th className="p-4">Subject</th><th className="p-4 text-right rounded-tr-xl">Action</th></tr></thead>
//                     <tbody>
//                       {facultyList.length === 0 ? <tr><td colSpan={4} className="p-6 text-center text-slate-500">No faculty found.</td></tr> : 
//                         facultyList.map(f => (
//                         <tr key={f.email} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
//                           <td className="p-4 font-bold text-white whitespace-nowrap">{f.name}</td>
//                           <td className="p-4 whitespace-nowrap text-slate-300">{f.email}</td>
//                           <td className="p-4 text-purple-400 whitespace-nowrap">{f.subject_name}</td>
//                           <td className="p-4 text-right"><button onClick={() => deleteRecord('faculty', 'email', f.email)} className="text-slate-500 hover:text-red-400 bg-slate-950 hover:bg-red-500/10 p-2 rounded-lg transition-all"><Trash2 size={18}/></button></td>
//                         </tr>
//                       ))}
//                     </tbody>
//                   </table>
...
//                  <span className="text-xs text-slate-400 bg-slate-950 px-4 py-2 rounded-full border border-slate-800 tracking-wider">READ / DELETE MODE</span>
//                </div>
//                <div className="overflow-x-auto w-full rounded-xl">
//                  <table className="w-full text-left min-w-[500px]">
//                    <thead><tr className="text-slate-400 border-b border-slate-800 bg-slate-950"><th className="p-4 rounded-tl-xl">Roll No</th><th className="p-4">MAC Address</th><th className="p-4 text-right rounded-tr-xl">Action</th></tr></thead>
//                    <tbody>
//                      {studentsList.length === 0 ? <tr><td colSpan={3} className="p-6 text-center text-slate-500">No students registered yet.</td></tr> :
//                        studentsList.map(s => (
//                        <tr key={s.mac_address} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
//                          <td className="p-4 font-bold whitespace-nowrap">{s.roll_number}</td>
//                          <td className="p-4 font-mono text-blue-300 text-sm whitespace-nowrap">{s.mac_address}</td>
//                          <td className="p-4 text-right"><button onClick={() => deleteRecord('students', 'mac_address', s.mac_address)} className="text-slate-500 hover:text-red-400 bg-slate-950 hover:bg-red-500/10 p-2 rounded-lg transition-all"><Trash2 size={18}/></button></td>
//                        </tr>
//                      ))}
//                    </tbody>
//                  </table>
...
//     return (
//       <div className="min-h-screen bg-slate-950 text-slate-50 p-4 md:p-8">
//         <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 mb-8 text-center md:text-left">
//           <div>
//             <h1 className="text-2xl font-black text-blue-400">Welcome, {facultyName || "Professor"}</h1>
//             <p className="text-sm text-slate-400 mt-1">Subject: <strong className="text-white">{facultySubject}</strong></p>
//           </div>
//           <button onClick={() => supabase.auth.signOut()} className="text-sm bg-slate-800 px-6 py-2 rounded-xl hover:bg-slate-700 transition-all w-full md:w-auto font-medium">Logout</button>
//         </div>

//         <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-center md:justify-start gap-2 mb-8 border-b border-slate-800 pb-4">
//           <button onClick={() => setFacultyTab('scanner')} className={`px-6 py-3 font-bold rounded-xl flex items-center justify-center gap-2 transition-all w-full md:w-auto ${facultyTab === 'scanner' ? 'bg-blue-600' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}><Upload size={18}/> Scanner</button>
//           <button onClick={() => { setFacultyTab('reports'); fetchFacultyReports(facultySubject!); }} className={`px-6 py-3 font-bold rounded-xl flex items-center justify-center gap-2 transition-all w-full md:w-auto ${facultyTab === 'reports' ? 'bg-blue-600' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}><BarChart2 size={18}/> Reports</button>
//         </div>

//         <div className="max-w-4xl mx-auto">
//           {facultyTab === 'scanner' && (
//             <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-10 shadow-xl">
//               <div className="border-2 border-dashed border-slate-700 rounded-2xl p-8 md:p-16 text-center cursor-pointer hover:border-blue-500/50 bg-slate-950/50 transition-colors">
//                 <input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} className="hidden" id="csv" />
//                 <label htmlFor="csv" className="cursor-pointer block text-slate-300 font-medium break-all px-4">{file ? file.name : `Upload ESP8266 CSV for ${facultySubject}`}</label>
//               </div>
//               <button onClick={handleFacultyUpload} disabled={loading} className="mt-8 w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold transition-all shadow-lg text-lg">Process Attendance</button>
//               {message && <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl text-blue-300 flex items-center justify-center gap-2 font-medium"><CheckCircle2 size={18}/> {message}</div>}
//             </div>
//           )}
...
//                 <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 text-center md:text-left flex flex-col justify-center">
//                   <p className="text-slate-400 text-sm font-medium uppercase tracking-wider mb-1">Total Scanned Records</p>
//                   <h3 className="text-4xl font-black text-blue-400">{reportData.length}</h3>
//                 </div>
//                 <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 flex items-center justify-center">
//                   <button onClick={downloadCSV} className="w-full py-4 bg-green-600/10 text-green-400 border border-green-600/30 hover:bg-green-600 hover:text-white rounded-xl font-bold flex justify-center items-center gap-2 transition-all text-lg">
//                     <Download size={20}/> Export CSV Report
//                   </button>
//                 </div>
//               </div>
//               <div className="bg-slate-900 p-4 md:p-8 rounded-3xl border border-slate-800">
//                 <div className="overflow-x-auto w-full max-h-[500px] rounded-xl">
//                   <table className="w-full text-left min-w-[400px]">
//                     <thead className="sticky top-0 bg-slate-950 shadow-md">
//                       <tr className="text-slate-400 border-b border-slate-800"><th className="p-4 rounded-tl-xl">Date</th><th className="p-4">Roll</th><th className="p-4 rounded-tr-xl">MAC</th></tr>
//                     </thead>
//                     <tbody>
//                       {reportData.length === 0 ? <tr><td colSpan={3} className="p-6 text-center text-slate-500">No attendance data found.</td></tr> :
//                         reportData.map((r, i) => (
//                         <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30 text-sm transition-colors">
//                           <td className="p-4 whitespace-nowrap text-slate-300">{r.date}</td>
//                           <td className="p-4 font-bold text-white whitespace-nowrap">{r.roll_number}</td>
//                           <td className="p-4 font-mono text-slate-500 whitespace-nowrap">{r.mac_address}</td>
//                         </tr>
//                       ))}
//                     </tbody>
//                   </table>
...
//     return (
//       <div className="min-h-screen bg-slate-950 text-slate-50 p-4 md:p-8">
//         <div className="max-w-xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 mb-8 text-center md:text-left">
//           <h2 className="text-2xl font-black text-blue-400">Student Portal</h2>
//           <button onClick={() => supabase.auth.signOut()} className="text-sm bg-slate-800 px-6 py-2 rounded-xl hover:bg-slate-700 transition-all w-full md:w-auto font-medium">Logout</button>
//         </div>

//         <div className="max-w-xl mx-auto flex flex-col md:flex-row justify-center md:justify-start gap-2 mb-8 border-b border-slate-800 pb-4">
//           <button onClick={() => setStudentTab('profile')} className={`px-6 py-3 font-bold rounded-xl transition-all w-full md:w-auto ${studentTab === 'profile' ? 'bg-blue-600' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}>My Device</button>
//           <button onClick={() => setStudentTab('attendance')} className={`px-6 py-3 font-bold rounded-xl transition-all w-full md:w-auto ${studentTab === 'attendance' ? 'bg-blue-600' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}>My Attendance</button>
//         </div>

//         <div className="max-w-xl mx-auto">
//           {studentTab === 'profile' && (
//             <div className="space-y-6">
//               <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-4 shadow-lg text-center md:text-left">
//                 <div className="bg-blue-600/20 p-4 rounded-full text-blue-400"><Smartphone size={28} /></div>
//                 <div>
//                   <p className="text-sm text-slate-400 mb-1">Branch: {branch} | Batch: 20{batch}</p>
//                   <p className="font-black text-2xl text-white">Roll No: {rollNo}</p>
//                 </div>
//               </div>
//               <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-10 shadow-xl">
//                 <p className="text-slate-400 mb-6 text-sm text-center md:text-left">Enter your device MAC address (Settings &gt; About Phone &gt; Status).</p>
//                 <input value={macInput} onChange={(e) => setMacInput(e.target.value.toUpperCase())} placeholder="AA:BB:CC:DD:EE:FF" className="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl mb-6 text-white uppercase font-mono outline-none focus:border-blue-500 text-center md:text-left tracking-widest" />
//                 <button onClick={handleStudentLink} disabled={loading} className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold transition-all shadow-lg text-lg">
//                   {loading ? "Linking..." : "Save Registration"}
//                 </button>
//                 {message && <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl text-blue-300 text-sm flex items-center justify-center gap-2 font-medium"><CheckCircle2 size={16}/> {message}</div>}
//               </div>
//             </div>
//           )}
          
//           {studentTab === 'attendance' && (
//              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-10 md:p-16 text-center shadow-xl">
//                <BookOpen size={56} className="text-slate-700 mx-auto mb-6" />
//                <h3 className="text-xl font-bold mb-3 text-white">Attendance Syncing</h3>
//                <p className="text-slate-400 text-sm max-w-xs mx-auto leading-relaxed">Once your device is registered and scanned by faculty, your subject-wise attendance logs will appear here.</p>
//              </div>
//           )}
//         </div>
//       </div>
